        ...
        for attempt in range(1, self.retries + 2):  # +1 for the initial attempt
            ...
            try:
                ...
                if self.input_schema:
                    try:
                        input_data = self.input_schema(**context.data)
                ...
                if self.output_schema and step_result is not None:
                    try:
                        ...
                        raise ValueError(f"Invalid output: {str(e)}") from e
                
                # Update step result
                result.status = WorkflowStepStatus.COMPLETED
                result.result = step_result if isinstance(step_result, dict) else {"result": step_result}
                ...
                self.logger.warning(
                    f"Attempt {attempt} failed for step '{self.name}'",
                    exc_info=True
                )
...
class Workflow:
    ...
    def add_step(
        ...
        step = WorkflowStep(
            ...
            retry_delay=retry_delay if retry_delay is not None else step_metadata.get("retry_delay", 1.0),
    ...
    async def run(
        ...
        try:
            ...
            for step in self.steps:
                ...
                if missing_deps:
                    error_msg = f"Step '{step.name}' is missing dependencies: {', '.join(missing_deps)}"
                ...
                try:
                    ...
                    if self.on_error == "stop":
                        ...
                        context.status = WorkflowStatus.CANCELLED
                        self.logger.error(f"Step '{step.name}' failed, cancelling workflow: {str(e)}")
                        return context
