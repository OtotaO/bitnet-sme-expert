...
class QueryRequest(BaseModel):
    ...
    domain: Optional[ExpertDomain] = Field(
        None, 
        description="Specific expert domain to query (auto-detected if not provided)"
    )
    max_tokens: int = Field(150, ge=10, le=2048, description="Maximum number of tokens in the response")
    temperature: float = Field(0.7, ge=0.0, le=2.0, description="Sampling temperature")
    top_p: float = Field(0.9, ge=0.0, le=1.0, description="Nucleus sampling parameter")
    context: Optional[Dict[str, Any]] = Field(
        None,
        description="Additional context for the expert to consider"
    )
...
class CollaborateRequest(BaseModel):
    ...
    max_tokens: int = Field(100, ge=10, le=1024, description="Maximum number of tokens per expert response")
    temperature: float = Field(0.7, ge=0.0, le=2.0, description="Sampling temperature")
    top_p: float = Field(0.9, ge=0.0, le=1.0, description="Nucleus sampling parameter")
    context: Optional[Dict[str, Any]] = Field(
        None,
        description="Additional context for the experts to consider"
    )
...
class FeedbackRequest(BaseModel):
    ...
    corrections: Optional[Dict[str, Any]] = Field(
        None,
        description="Corrections to the expert's response"
    )

class SearchRequest(BaseModel):
    """Request model for searching across experts."""
    query: str = Field(..., description="Search query")
    domains: Optional[List[ExpertDomain]] = Field(
        None,
        description="Expert domains to search within (all domains if not specified)"
    )
